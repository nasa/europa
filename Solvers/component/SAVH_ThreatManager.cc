...
//     };
    ...
    bool DecisionOrder::operator()(const InstantId& a, const InstantId& b, LabelStr& explanation) const {
      ...
      for(std::list<InstantComparator*>::const_iterator it = m_cmps.begin(); it != m_cmps.end(); ++it) {
        ...
        if((*cmp)(a, b)) {
          ...
          debugMsg("ThreatManager:betterThan", "b better than a");
          return false;
        }
        debugMsg("ThreatManager:betterThan", "a == b.  Checking next comparator.");
      }
      return false;
    }
    ...
    void DecisionOrder::addOrder(InstantComparator* cmp) {
      ...
      debugMsg("ThreatManager:betterThan", "Adding comparator " << cmp->toString());
    ...
    ThreatManager::ThreatManager(const TiXmlElement& configData) : FlawManager(configData), m_preferUpper(false), m_preferLower(false) {
    ...
    bool ThreatManager::betterThan(const EntityId& a, const EntityId& b, LabelStr& explanation) {
    ...
    public:
      ThreatIterator(ThreatManager& manager) : FlawIterator(manager) {
