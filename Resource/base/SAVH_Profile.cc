...
namespace EUROPA {
  namespace SAVH {
    ...
    ProfileId ProfileFactory::createInstance(const LabelStr& name, const PlanDatabaseId db, const FVDetectorId detector,
      ...
      checkError(it != factoryMap().end(), "No factory registered for profile '" << name.toString() << "'");
      
      ProfileFactory* factory = it->second;
    ...
    std::map<double, ProfileFactory*>& ProfileFactory::factoryMap() {
      static std::map<double, ProfileFactory*> sl_factoryMap;
