...
public abstract class GenericGanttView extends ViewPart implements ISelectionProvider {
	...
	/** Set values for oddBg, evenBg, smallGrid, boldGrid */
	...
	private void doUpdate() {
		...
		for (int i = 0; i < gmodel.getResourceCount(); i++) {
			IGanttResource r = gmodel.getResource(i);
			LinePanel line = null;
			if (r != null) {
				...
				List<IGanttActivity> all = gmodel.getActivities(i);
				if (skipEmptyObjects && all.isEmpty())
					continue;
				for (IGanttActivity act : all) {
					tline.addToken(new ActivityWidget(act,
							ActivityWidget.DEFAULT_COLOR, this));
				}
				line = tline;
			}
			if (index++ % 2 == 0)
	...
	public void doLayout() {
		...
		IGanttModel model = getGanttModel();
		int[] hor = {model.getStart(), model.getEnd()};
		int y = 0;
