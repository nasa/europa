/**
 * @mainpage EUROPA
 * EUROPA is a library for building automated planning applications. The library is structured to allow easy integration, customization and extension. It provides:
 * @li A <b>Plan Database</b> to manage the data associated with plans and planning problems. The database ensures compliance of the plan with the domain model.
 * @li A <b>Planner</b> to solve planning problems in the plan database. The planner automatically finds and fixes flaws in the plan database.
 * @li A set of <b> Application Development Tools</b>. This include a high-level modeling language accompanied by a parser and compiler. It also includes an interactive graphical debugging tool (PlanWorks) for inspection and troubleshooting plans and planners.
 *
 * EUROPA has been used in a wide variety of applications, including:
