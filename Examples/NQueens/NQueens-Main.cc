...
void printQueens(PSEngine& psengine)
{
	PSList<PSVariable*> l = psengine.getGlobalVariables();	
	
	int N=8;
	for (int i=0;i<N;i++) {
		PSVariable* v = l.get(i);
		debugMsg("Main",v->getName() << " = " << v->toString());
	}    			
	
	debugMsg("Main",std::endl);
}

void initQueens(PSEngine& psengine)
{	
	PSList<PSVariable*> l = psengine.getGlobalVariables();	
	PSVarValue value = PSVarValue::getInstance(1);
