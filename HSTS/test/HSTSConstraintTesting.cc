...
namespace EUROPA {
  ...
  bool HSTSReadTestCases(std::string file, std::list<HSTSConstraintTestCase>& testCases) {
	...
	tCS.get(ch);
      ...
      testCases.push_back(HSTSConstraintTestCase(constraintName, file, line++, domains));
    } // while tCS.good() && !tCS.eof()
    return(tCS.eof());
  }

  bool HSTSExecuteTestCases(const ConstraintEngineId& engine,
                                std::list<HSTSConstraintTestCase>& testCases) {
