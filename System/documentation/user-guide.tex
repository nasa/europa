...
\> \} \\
\> pr\=edicate Stow \{ \\
\> \> eq(stow\_speed, duration); \\
\> \} \\
\> pr\=edicate Unstow \{ \\
\> \> eq(unstow\_speed, duration); \\
\> \} \\
\> predicate Stowed \{ \} \\
\> predicate Unstowed \{ \} \\
\} 
...
predicates can have argument variables such as ``position\_speed'' from the
``Position'' predicate.  Predicates can also impose constraints by
definition on its arguments whether they're implicit, such as duration, or
explicit, such as ``position\_speed''. In this case, we equal the
corresponding speeds to the duration of each of the ``Position'', ``Stow'',
or ``Unstow'' predicates. 
...
for Instruments, Navigators, and Batteries.
