...
protected:
        ...
        class StringToBuffer
        {
          public:
                ...
                char* buffer;
        private:
                StringToBuffer& operator=(const StringToBuffer&);
                StringToBuffer(const StringToBuffer&);
        };
...
class TiXmlNode : public TiXmlBase
{
  ...
  friend class TiXmlElement;
 private:
  TiXmlNode& operator=(const TiXmlNode&);
...
private:
  TiXmlAttribute& operator=(const TiXmlAttribute&);
  TiXmlAttribute(const TiXmlAttribute&);
public:
  /// Construct an empty attribute.
  TiXmlAttribute()
    : TiXmlBase(), document(NULL), name(), value(), prev(NULL), next(NULL) {}
        
#ifdef TIXML_USE_STL
  /// std::string constructor.
  TiXmlAttribute( const std::string& _name, const std::string& _value ) 
    : TiXmlBase(), document(NULL), name(_name), value(_value), prev(NULL),
      next(NULL) {
  }
#endif
...
public:
  ...
  TiXmlDocument( const std::string& documentName ) :
    TiXmlNode( TiXmlNode::DOCUMENT ), error(false), errorId(0), errorDesc(), tabsize(4),
    errorLocation() {
    value = documentName;
  }
#endif
