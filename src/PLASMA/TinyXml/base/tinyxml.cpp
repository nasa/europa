...
// End strange bug fix. -->
  ...
  TiXmlNode::TiXmlNode( NodeType _type ) 
    : parent(NULL), type(_type), firstChild(NULL), lastChild(NULL), value(), prev(NULL),
      next(NULL), userData(NULL) {
  }
...
TiXmlNode* TiXmlElement::Clone() const
{
  ...
  TiXmlDocument::TiXmlDocument() 
    : TiXmlNode( TiXmlNode::DOCUMENT ), error(false), errorId(0), errorDesc(), 
      tabsize(4), errorLocation() {
    ClearError();
  }
...
TiXmlNode* TiXmlUnknown::Clone() const
{
  ...
  TiXmlAttributeSet::TiXmlAttributeSet() : sentinel() {
