...
Profile::~Profile() {
  ...
  delete static_cast<ConstraintEngineListener*>(m_removalListener);
...
void Profile::handleTransactionAdded(const TransactionId) {
    ...
    if((*it)->isVariableOf(t->time())) {
      removals.push_back(std::make_pair(*it, 
                                        std::distance((*it)->getScope().begin(), 
                                                      std::find((*it)->getScope().begin(),
                                                                (*it)->getScope().end(),
                                                                t->time()))));
    }
  }
  for(std::vector<std::pair<ConstraintId, unsigned long> >::const_iterator it = removals.begin();
      it != removals.end(); ++it)
    handleConstraintMessage(it->first, t->time(), static_cast<unsigned int>(it->second), false);
...
void Profile::handleTransactionRemoved(const TransactionId) {
  //by default, create an iterator over all time
  if(m_recomputeInterval.isValid()) {
...
void Profile::transactionTimeChanged(const TransactionId e,
                                     const DomainListener::ChangeType& change) {
  ...
  eint startTime = static_cast<eint>(e->time()->lastDomain().getLowerBound());
  eint endTime = static_cast<eint>(e->time()->lastDomain().getUpperBound());
  switch(change) {
    ...
    case DomainListener::SET_TO_SINGLETON: {
      ...
      for(std::vector<eint>::iterator emptyIt = emptyInstants.begin();
          emptyIt != emptyInstants.end(); ++emptyIt) {
        std::map<eint, InstantId>::iterator instIt = m_instants.find(*emptyIt);
        checkError(instIt != m_instants.end(),
                   "Computed empty instant at time " << *emptyIt << " but no such instant exists.");
        InstantId inst = instIt->second;
        debugMsg("Profile:handleTimeChanged", "Removing instant at time " << inst->getTime());
        condDebugMsg(inst->getTransactions().empty(),
                     "Profile:handleTimeChanged", "because it has no transactions.");
        condDebugMsg(!containsChange(inst),
                     "Profile:handleTimeChanged", "because it does not mark a change.");
        m_instants.erase(*emptyIt);
        m_detector->notifyDeleted(inst);
    ...
    case DomainListener::RELAXED: {
      debugMsg("Profile:handleTimeChanged",
               "Handling relaxation of transaction " << e << " at time " <<
               e->time()->toString() << " with quantity " << e->quantity()->toString());
  ...
  handleTransactionTimeChanged(e, change);
    ...
    void Profile::handleTransactionTimeChanged(const TransactionId,
    ...
    void Profile::handleTransactionQuantityChanged(const TransactionId,
                                                   const DomainListener::ChangeType&) {
      //by default, create an iterator over all time
      if(m_recomputeInterval.isValid()) {
...
void Profile::handleConstraintMessage(const ConstraintId c,
                                      const ConstrainedVariableId,
...
void Profile::temporalConstraintAdded(const ConstraintId c,
                                      const ConstrainedVariableId var,
                                      unsigned int argIndex) {
  handleConstraintMessage(c, var, argIndex, true);
}

void Profile::handleTemporalConstraintAdded(const TransactionId, const unsigned int,
                                            const TransactionId, const unsigned int) {
  if(m_recomputeInterval.isValid())
    delete static_cast<ProfileIterator*>(m_recomputeInterval);
  m_recomputeInterval = (new ProfileIterator(getId()))->getId();
}

void Profile::temporalConstraintRemoved(const ConstraintId c,
                                        const ConstrainedVariableId var,
                                        unsigned int argIndex) {
  handleConstraintMessage(c, var, argIndex, false);
}

void Profile::handleTemporalConstraintRemoved(const TransactionId, const unsigned int,
                                              const TransactionId, const unsigned int) {
  if(m_recomputeInterval.isValid())
    delete static_cast<ProfileIterator*>(m_recomputeInterval);
  m_recomputeInterval = (new ProfileIterator(getId()))->getId();
...
void Profile::addInstantsForBounds(const TransactionId t) {
  ...
  eint last =  static_cast<eint>(t->time()->lastDomain().getUpperBound());
...
void Profile::ConstraintRemovalListener::notifyRemoved(const ConstraintId constr) {
  if(m_profile->hasConstraint(constr)) {
    ...
    for(std::vector<ConstrainedVariableId>::const_iterator it = constr->getScope().begin();
        it != constr->getScope().end(); ++it, ++index)
